...
async function main() {
  ...
  const missing = items.filter((item) => item.type === 'photo' && !item.thumbnailKey);

  console.log(`${items.length} total item(s), ${missing.length} photo(s) missing a thumbnail.`);
  ...
  await runPool(missing, CONCURRENCY, async (item) => {
    try {
      ...
      if (!thumbnailKey) {
        failed++;
        console.warn(`  Thumbnail generation returned nothing for ${item.filename}`);
        return;
      }
  ...
  console.log(`Backfilled ${done} thumbnail(s), ${failed} failure(s).`);
}
